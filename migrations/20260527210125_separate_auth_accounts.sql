...
DELETE FROM users WHERE id IN (SELECT id FROM accounts);
...
UPDATE users u SET email = a.email, password_hash = a.password_hash
FROM accounts a WHERE u.account_id = a.id;

-- Re-insert auth-only records back as empty user rows
INSERT INTO users (id, email, password_hash, created_at, updated_at)
SELECT a.id, a.email, a.password_hash, a.created_at, a.created_at
FROM accounts a
WHERE a.id NOT IN (SELECT account_id FROM users WHERE account_id IS NOT NULL);
